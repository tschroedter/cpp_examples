...
ADC0832::ADC0832(adcchannel number_of_channels, wiringpipin pin_cs, wiringpipin pin_dio, wiringpipin pin_clk)
        : m_number_of_channels(number_of_channels),
          m_pin_cs(pin_cs),
          m_pin_dio(pin_dio),
          m_pin_clk(pin_clk) {
...
void ADC0832::validate_channel(adcchannel channel) const {
    if (channel >= m_number_of_channels || channel < 0) {
        throw Common::Exceptions::ADCChannelInvalidException(
                "Invalid channel '" + std::to_string(channel) + "'! Channel must 0 <= channel < "
