...
SerialShiftRegisterManager::SerialShiftRegisterManager(IThreadSafe74HC595_SPtr ssr)
        : m_ssr(ssr) {
    if (m_ssr == nullptr) {
        throw Common::Exceptions::ArgumentInvalidException(
                "Can't create SerialShiftRegisterManager because 'm_ssr' is null!", "m_ssr");
    }
}

void SerialShiftRegisterManager::initialize(wiringpipin pin_ds, wiringpipin pin_sh_dp, wiringpipin pin_sh_cp) {
